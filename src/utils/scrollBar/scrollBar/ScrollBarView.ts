...
import { EventEmitter, FederatedPointerEvent } from 'pixi.js';
import { remap } from '../../../../../launcher/src/utils/math';
...
export class ScrollBarView extends SliderView {
  ...
  public scrollToItem(index: number): void {
    const items = this.contents.target.children;
    ...
    if (index >= 0 && index < items.length) {
      ...
      console.log(this._maxPosition, itemHeight, margin);

      gsap.killTweensOf(this.contents.target);
      gsap.to(this.contents.target, {
        duration: 0.2,
        y: this._maxPosition / 2 - itemHeight / 2 - margin,
      });
      ...
      SliderViewUtil.setPosition(
        ...
        this.isHorizontal,
        this.contents.container.height / 2 - itemHeight / 2 - margin
      );
       */
    }
  }
