import { Application, BaseTexture, Container, IRenderer, MIPMAP_MODES, Renderer, settings } from "pixi.js";
...
export class AppService {
    ...
    public setPixelArtMode(value: boolean): void {
        BaseTexture.defaultOptions.mipmap = value ? MIPMAP_MODES.OFF : MIPMAP_MODES.ON;
    }
    ...
    public resize = (): void => {
        ...
        if (innerWidth !== this.width || innerHeight !== this.height) {
            ...
            this.scale = Math.min(
                this.maxScale,
                Math.min(this.width / this.initialWidth, this.height / this.initialWidth),
            );
